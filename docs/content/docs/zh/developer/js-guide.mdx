...
Redefine 在 `source/js/main.js` 中直接处理这些生命周期：

- 初次触发 `DOMContentLoaded` 时，初始化全局监听器和当前页面。
- Swup 触发 `page:view` 时，初始化新页面。
- Swup 替换内容前，取消旧页面作用域内的任务。
- `visit:start` 时，更新主题界面的导航状态。
...
`pnpm dev` 会使用 esbuild 监听浏览器端 JavaScript，并生成本地链接目录 `dev/site/themes/redefine/`。浏览器会加载 `source/js/build/dev/` 下未压缩的开发 bundle；修改 JavaScript 后需要手动刷新页面。请勿通过生成的链接目录或构建目录修改文件。

请勿手动修改：

- `source/js/build/**`（生成且被忽略的构建输出目录）

提交到仓库的运行时资源位于 `source/assets/**`。

常见目录说明：

- `source/js/main.js`：主入口文件，建议在此接入新功能
- `source/js/layouts/`：页面布局相关功能（如目录、导航、首页横幅）
- `source/js/tools/`：工具类功能（搜索、图片查看器等）
- `source/js/plugins/`：可选插件集成（mermaid、typed 等）
...
例如：`scripts/config-export.js` 用于将主题配置导出到浏览器端全局变量
...
要求：每次执行时都假设 DOM 是新加载的，并使用新的页面作用域

## 尽量避免在 EJS 模板内写 `<script>`

推荐做法：

- 将 JavaScript 代码写在 `source/js/**` 目录下
...
- `visit:start`：开始切换页面
- `content:replace`：即将替换容器 DOM，先取消旧页面作用域内的任务
