...
When enabled, the toolbar percentage animates between rounded integer values as
you scroll. It updates without animation when the browser's reduced-motion
preference is enabled. The top progress bar remains a separate continuous
indicator controlled by `global.scroll_progress.bar`.
