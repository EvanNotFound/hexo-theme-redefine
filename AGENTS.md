...
## Environment & Tooling
- Package manager: pnpm (the repository includes npm-compatible package metadata).
...
- If you touch `source/js/**` or `source/css/**`, rerun `pnpm run build` for verification. Build outputs are committed by CI on `dev`/`main` after merge.
...
## CI/Automation Notes
- GitHub Actions build workflow runs `pnpm run build` on `main`/`dev` changes to JS/CSS.
...
- Run `pnpm run build` to refresh compiled assets.
...
- Rebuild with `pnpm run build` for verification.
