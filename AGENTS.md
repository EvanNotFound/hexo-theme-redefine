...
- Pre-commit hook runs: `npm run build` (see `.husky/pre-commit`).
...
- If you touch `source/js/**` or `source/css/**`, rerun `npm run build` and include updated build output.
...
- Verify the output files were updated if the change affects runtime assets.
